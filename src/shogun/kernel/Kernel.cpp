	...
	else
	{
		...
		if (num>0)
		{
			...
			for (int32_t t=0; t<num_threads; t++)
			{
				...
				cache_multiple_kernel_row_helper(&params[t]);
			}
		}
		else
			num_threads=-1;
...
SGMatrix<T> CKernel::get_kernel_matrix()
{
	...
	{
		params[0].kernel = this;
		params[0].result = result;
		params[0].start = compute_row_start(t*step, n, symmetric);
		params[0].end = m;
		params[0].total_start=t*step;
		params[0].total_end=total_num;
		params[0].n=n;
		params[0].m=m;
		params[0].symmetric=symmetric;
		params[0].verbose=false;
		CKernel::get_kernel_matrix_helper<T>((void*)&params[0]);
	}
